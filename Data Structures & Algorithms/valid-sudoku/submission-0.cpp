...
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        vector<vector<unordered_set<char>>> track_block(3, vector<unordered_set<char>>(3));
        ...
        for (int r = 0; r < 9; ++r) {
            for (int c = 0; c < 9; ++c) {
                ...
                it = track_block[r/3][c/3].find(ch);
                if (it == track_block[r/3][c/3].end()) {
                    track_block[r/3][c/3].insert(ch);
                } else { return false; }
            }
        }
